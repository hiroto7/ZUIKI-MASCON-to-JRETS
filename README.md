...
3. main.py を実行する
   ...
   - 東武・西武の車両を運転する場合は、車両に合わせて `--profile` を指定する
     ...
     ```
   - 標準車両プロファイルで実験的なEBランプ表示を使う場合は `--experimental-eb-lamp` を指定する（東武・西武プロファイルでは表示されません）
     ```bash
     uv run python main.py --verbose --experimental-eb-lamp
     ```
...
デフォルトのマッピングは、通常版（ダウンロード版）のズイキマスコンを使用したときと同じ挙動となるよう設定しています。例えば、公式サポートで「警笛（2段目）」に対応するAボタンは、キーボード操作で同じ意味となる <kbd>backspace</kbd> キーにマッピングされます。同様に、「連絡ブザースイッチ」に対応する左ボタンは、 <kbd>b</kbd> キーにマッピングされます。なおHOMEボタンとキャプチャーボタンに限っては、GeForce Nowアプリのキーボードショートカットにマッピングされています。

デフォルトのマッピングの場合、公式サポートと同じ挙動となるのは運転画面のみです。メニュー画面では左右移動などの操作ができないため、キーボードやマウスを使用する必要があります。

このマッピングは mascon_controller.py 内の `BUTTON_MAPPINGS` を編集することでカスタマイズできます。ただしZLボタンにはマッピングを設定できません。ボタン割り当てを入れ替える場合は、キーだけでなく `resets_eb_timer` も含めた `ButtonMapping` ごと移動してください。
